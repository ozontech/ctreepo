...
__all__ = ("CiscoIOSXEBGP",)


@register_rule(Platform.CISCO_IOSXE)
class CiscoIOSXEBGP(CTreePostProc):
    ...
    def _delete_peers(cls, ct: CTree) -> None:
        ...
        for node in ct.children.values():
            if node.line.startswith("address-family ") and len(node.children) != 0:
    ...
    def _delete_empty_af(cls, ct: CTree) -> None:
        ...
        to_delete = [n for n in ct.children.values() if n.line.startswith("address-family ") and len(n.children) == 0]
        for node in to_delete:
            node.delete()
