...
- [CTreePO - Configuration Tree Patch Overview](#ctreepo---configuration-tree-patch-overview)
  ...
  - [Пост-обработка diff'а](#пост-обработка-diffа)
...
## Пост-обработка diff'а

### `cisco_iosxe`

- `CiscoIOSXEBGP`:
  - сортировка внутри bgp секции
  - обработка удаления пиров/групп
  - удаление пустых секций address-family
- `CiscoIOSXENoValue`:
  - удаление команд и соответствующих им undo-команд, с `<<no-value>>`

### `arista_eos`

- `AristaEOSBGP` **! требует уточнения !**:
  - сортировка внутри bgp секции
  - обработка удаления пиров/групп
  - удаление пустых секций address-family
- `AristaEOSNoValue`:
  - удаление команд и соответствующих им undo-команд, с `<<no-value>>`
- `AristaEOSPrefixList`:
  - удаление undo-тега с нод
  - если entry меняется, то поднять её undo-вариант перед изменением (arista не дает переписывать поверх существующих записей, поэтому сначала удалить, затем добавить)
