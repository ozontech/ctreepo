...
from ctreepo.postproc.huawei_vrp import HuaweiVRPNoValue
    ...
    [
        (
            ...
            dedent(
                ...
                snmp-server community {settings.NO_VALUE} ro
                #
                undo snmp-server community public12345 ro
                #
                """,
            ).strip(),
            "",
        ...
        (
            ...
            dedent(
                ...
                snmp-server host 192.168.0.2 {settings.NO_VALUE} snmp bgp
                #
                undo snmp-server host 192.168.0.2 public12345 snmp bgp
                #
                undo snmp-server host 192.168.0.3 public12345 snmp bgp
                #
                """,
            ).strip(),
            dedent(
        ...
        (
            ...
            dedent(
                f"""
                aaa group server tacacs+ TACACS-GROUP
                 undo server-private 1.2.3.4 key 7 secret-key
                 server-private 1.2.3.4 key 7 {settings.NO_VALUE}
                #
                enable secret {settings.NO_VALUE}
                #
                undo enable secret some-secret-hash
                #
                """,
            ).strip(),
            "",
...
def test_pp_arista_eos_no_value(
    ...
    pp_diff = CTreeDiffer.diff(current, target, post_proc_rules=[HuaweiVRPNoValue])
